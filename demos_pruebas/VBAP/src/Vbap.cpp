...
Vbap::Vbap(int numspeak) {
	...
	out.open("data/intersections.txt");
	outcrossed.open("data/crossed.txt");
	selected.open("data/selected.txt");
	numIntersec = numInside = 0;
}
...
//					speakDist distanceij, distanceik, distancejk;
					//calculamos las distancias entre speakers
...
//					distanceij.i = distanceik.i = i;
//					distanceij.j = distancejk.i = j;
//					distanceik.j = distancejk.j = k;
//					distanceij.distance = distances[i][j];
//					distanceik.distance = distances[i][k];
//					distancejk.distance = distances[j][k];
//
...
//					if(find(distBetweenSpeakers.begin(), distBetweenSpeakers.end(), distanceik) 
//							== distBetweenSpeakers.end())
//						distBetweenSpeakers.push_back(distanceik);
//					if(find(distBetweenSpeakers.begin(), distBetweenSpeakers.end(), distancejk) 
//							== distBetweenSpeakers.end())
//						distBetweenSpeakers.push_back(distancejk);
				} else { //borramos la memoria creada
//					outcrossed << "Área Pequeña?" << vol << endl
//							<< trian->toString() << endl;
					delete trian;
				}
			}
...
//	for(unsigned int i = 0; i < distBetweenSpeakers.size(); i++){
//		speakDist a = distBetweenSpeakers.at(i);
//		cout << "Distancia: "<< a.distance << "\t Speaker: \t("<< a.i;
//		cout << ","<< a.j << ")" << endl;
//	}	
}
...
void Vbap::removeTriangles() {
	...
	selected << "Triángulos antes de borrar: " << activeTriangles.size() << endl;
	int borrados = 0;
	...
	out << "Número de Intersecciones: " << numIntersec << endl;
	outcrossed << "Número de Speakers dentro Triangulos: " << numInside << endl;

	selected << "Deberían haberse borrado: " << borrados << endl;
	selected << "Restantes: " << activeTriangles.size() << endl;

	for (it1 = activeTriangles.begin(); it1 != activeTriangles.end(); it1++)
		selected << (*it1)->toString() << endl;
...
//			normalize(res);
			...
			if (res[0] >= -0.001f && res[1] >= -0.001f && res[2] >= -0.001f) {
				result = true;
				//				cout << "!" << endl;
				outcrossed << "Speaker dentro de triángulo: "<< a->toString();
				outcrossed << totalSpeakers[i]->toString();
				outcrossed << "Vector Resultado: " << res << endl << endl;
				numInside++;
				cont++;
			}
		}
	...
	if(result)
		outcrossed << "Hubo " << cont << " Intersecciones" << endl << endl << "!" << endl;
	return result;
}
...
//		if (outcrossed.is_open())
//			outcrossed << "Lado Cruzado: " << b->toString() << endl;
		return true;
	}
	return false;
}
...
//		omitimos valores cerca a los puntos de cruce
		...
		if((totalij <= 0.01 && totalmn <= 0.01) ||
			...
			if(out.is_open()) {
				out << "Hubo Intersección" << endl;
				out << "Speakers: (" << a.i << "," << a.j << "), (" << b.i << "," << b.j << ")" << endl;
				out << "Totalij = " << totalij << "; Totalmn = " << totalmn << " ";
				out << "TotalNegij = " << totalnegij << "; Totalnegmn = " << totalnegmn << endl;
				numIntersec++;
			}
		}
	}
...
pair<Vec3f, int*>* Vbap::selectTriangle(float x, float y, float z){
	...
	for(unsigned int i = 0; i < activeTriangles.size() && !salir; i++){
		...
		if(temp[0] >= -0.001f && temp[1] >= -0.001f && temp[2] >= -0.001f){
			cout << "Triángulo hallado para la fuente: " << endl;
