/**
 * @brief Programa de prueba para SoundAdminPortaudio usando la base del sistema desarrollado
 * mediante trackers y OpenGL
 * @author John Jairo Palma Robayo - Oscar Chavarro (jedilink@users.sourceforge.net)
 * @example TestTracker.cpp
 * */
//===========================================================================
...
//= Modificado por John Jairo Palma Robayo para trabajar con Uniandes3DAudio=
...
*/
SoundAdminPortaudio *admin2;

/**
 * Archivo del tracker 1
 * */
string fileTracker1;

/**
 * Archivo del tracker 2
 * */
string fileTracker2;

/**
 * Manejo de fuentes ambiente
 * */
bool ambientT1 = false, ambientT2 = false;
...
*/
void setSource(){
	admin2->updateSource("Tracker0", (float) posx, (float) posy, (float) posz);
	admin2->updateSource("Tracker1", (float) posx2, (float)posy2, (float)posz2);
}
...
*/
{
    ...
    switch ( tecla ) {
      ...
      case 'p':
        ...
        break;
	case 'u': 
		admin2->stopSource("Tracker0");
		break;
	case 's':
		admin2->stopSource("Tracker1");
		break;
	case '1':
		admin2->playSource("Tracker0", true);
		break;
	case '2':
		admin2->playSource("Tracker1", true);
		break;
	case 'c': case 'C':
		ambientT1 = !ambientT1;
		admin2->useSourceAsAmbient("Tracker0", ambientT1);
		break;
	case 'v': case 'V':
		ambientT2 = !ambientT2;
		admin2->useSourceAsAmbient("Tracker1", ambientT2);
		break;
      default:
...
int main(int argc, char **argv)
{
	...
	if(argc != 6){
		...
		printf("./Uniandes3DAudio <ruta Tracker> <Nombre Archivo Tracker 1> <Nombre Archivo Tracker 2> <numspeakers> <archivoConfiguracion>\n");
		exit(1);
	}
	else{
	...
	try{
		admin2 = SoundAdminPortaudio::getSoundAdminInstance();
		admin2->addFile("Tracker0", fileTracker1);
		admin2->addFile("Tracker1", fileTracker2);
		admin2->createSource("Tracker0", "Tracker0", ambientT1);
		admin2->createSource("Tracker1", "Tracker1", ambientT2);
		printf("fin?\n");
		
		admin2->playSource("Tracker0", true);
		admin2->playSource("Tracker1");
		
		admin2->setConfiguration(SoundAdminPortaudio::SR_44100_HZ, channels,
				argv[5], SoundAdminPortaudio::DISTRIBUTED);
	}
	catch (string str){
		printf("Error al trabajar en el sistema: %s", str.c_str());
		exit(EXIT_FAILURE);
	}
